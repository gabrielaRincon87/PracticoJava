...
@WebServlet("/pais") // Anotación que mapea este servlet a la URL "/usuario"
public class ControladorPais extends HttpServlet { // Declaración de la clase Controlador que extiende HttpServlet
    public Integer id = 1;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            String query = "SELECT id_pais, descripcion FROM paises";
            Statement statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery(query);  // Ejecutar la consulta y obtener los resultados

            List<Pais> paises = new ArrayList<>();  // Crear una lista para almacenar objetos Pelicula
            ...
            while (resultSet.next()) {
                ...
                paises.add(pais);  // Agregar el objeto Pelicula a la lista
            }

            ObjectMapper mapper = new ObjectMapper();  // Crear un objeto ObjectMapper para convertir objetos Java a JSON
            String json = mapper.writeValueAsString(paises);  // Convertir la lista de películas a formato JSON
