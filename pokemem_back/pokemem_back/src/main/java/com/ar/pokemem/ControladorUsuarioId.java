...
@WebServlet("/usuario/id") // Anotación que mapea este servlet a la URL "/usuario"

public class ControladorUsuarioId extends HttpServlet { // Declaración de la clase Controlador que extiende HttpServlet
    public Integer id = 1;
    ...
    protected void doPost (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                usuarios.add(usuarioRes);  // Agregar el objeto Pelicula a la lista
            }

            ObjectMapper mapperRes = new ObjectMapper();  // Crear un objeto ObjectMapper para convertir objetos Java a JSON
            String json = mapperRes.writeValueAsString(usuarios);  // Convertir la lista de películas a formato JSON

            response.setContentType("application/json");  // Establecer el tipo de contenido de la respuesta como JSON
            response.getWriter().write(json);
            // Configurar el código de estado de la respuesta HTTP como 201 (CREATED)
